...
function createMaterial(geometry) {
  ...
  return THREE.SceneUtils.createMultiMaterialObject(geometry, [lambert, basic]);
}
...
function removeAndAdd(item, value, camera, mesh, scene, controls) {
  ...
  if (isFont(item)) {
    mesh.pointer = createMaterial(
      new THREE[item.type]("THREE", Object.assign(textOptions, controls))
    );
  } else {
...
function getMeshValue(extend, name) {
  return {
    ...
    getValue: (item, camera, mesh) =>
      isFont(item) && textOptions[name] !== undefined
        ? textOptions[name]
        : mesh.children[0].geometry.parameters[name],
...
function initControls(item, camera, mesh, scene) {
  ...
  for (let i = 0; i < typeList.length; i++) {
    ...
    if (child) {
      controls[typeList[i]] = child.getValue(
        item,
        camera,
        mesh ? mesh.pointer : ""
      );
